  ...
  return (
    <div className={`min-h-screen flex flex-col ${bgClass}`}>
      ...
      <header className="flex items-center gap-2 px-3 py-2 border-b shrink-0" style={{ background: 'var(--color-card)', borderColor: 'var(--color-border)' }}>
        ...
        <button onClick={() => setBgMode(b => b === 'white' ? 'sepia' : b === 'sepia' ? 'warm' : 'white')} className="p-1.5 rounded-lg shrink-0"
      ...
      {/* Reading area */}
      <div className="flex-1" style={{ background: bgMode === 'sepia' ? '#f4ecd8' : bgMode === 'warm' ? '#faf3e8' : '#fffef9' }}>
        {isPdf ? (
          <iframe
            src={`${book.file_url}#view=FitH&toolbar=0`}
          ...
          <div className="flex items-center justify-center h-full p-4">
            <div className="text-center">
              <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>EPUB требует отдельного приложения.</p>
              <a href={book.file_url} className="mt-2 inline-block px-4 py-2 rounded-lg text-sm font-medium text-white"
      ...
      {pdfError && (
        <div className="text-center py-3 border-t shrink-0" style={{ background: '#fef2f2', borderColor: 'var(--color-border)' }}>
          <p className="text-sm mb-2" style={{ color: 'var(--color-danger)' }}>Не удалось загрузить PDF</p>
          <a href={book.file_url} className="text-sm font-medium" style={{ color: 'var(--color-link)' }}>Скачать напрямую</a>
        </div>
      )}
    </div>
  )
